...
function Cursor({ blink = true }: { blink?: boolean }) {
  return <span className={`inline-block w-2.5 h-5 bg-purple-500 dark:bg-purple-400 ml-0.5 align-middle ${blink ? 'animate-pulse' : ''}`} />
}
...
function TerminalLine({ prefix = '$', text, delay = 0, speed = 30, className = '' }: { prefix?: string; text: string; delay?: number; speed?: number; className?: string }) {
  ...
  return (
    <div className={`flex items-start gap-2 ${className}`}>
      <span className="text-purple-600 dark:text-purple-500 font-bold flex-shrink-0">{prefix}</span>
      <span className="text-purple-900/90 dark:text-purple-200/90 break-all">{displayed}{!done && <Cursor />}</span>
    </div>
  )
}
...
function ScanLines() {
  return (
    <div className="fixed inset-0 pointer-events-none z-50 opacity-[0.02] dark:opacity-[0.03]"
...
export default function Design5() {
  ...
  return (
    <LayoutWrapper>
      ...
      `}</style>
      <ScanLines />
      <div className="flex min-h-screen flex-col d5-mono bg-purple-50 dark:bg-[#0a0011] text-purple-900 dark:text-purple-200/90">
        <Navbar />
        <main className="flex-1 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto w-full py-8">
          {/* Terminal Window */}
          <div className="rounded-lg border border-purple-300 dark:border-purple-500/20 bg-white/80 dark:bg-[#0d0017]/80 backdrop-blur-sm overflow-hidden shadow-2xl shadow-purple-200/50 dark:shadow-purple-900/20">
            {/* Title bar */}
            <div className="flex items-center gap-2 px-4 py-2.5 bg-purple-100 dark:bg-purple-900/20 border-b border-purple-200 dark:border-purple-500/20">
              ...
              <span className="text-xs text-purple-500 dark:text-purple-400/60 ml-4">bgctub@it-club:~</span>
            </div>

            {/* Terminal content */}
            <div className="p-4 sm:p-6 space-y-3 text-sm sm:text-base min-h-[70vh]">
              {/* Boot sequence */}
              <div className="text-purple-400 dark:text-purple-500/50 text-xs space-y-1">
                ...
                <p>[BOOT] <span className="text-green-600 dark:text-green-400">All systems operational.</span></p>
                <p className="text-purple-300 dark:text-purple-500/30">────────────────────────────────────────────────────</p>
              </div>
              ...
              <TerminalLine prefix="#" text="HACK THE FUTURE" delay={1200} speed={60} className="text-2xl sm:text-3xl font-extrabold text-purple-600 dark:text-purple-400" />
              ...
              {showFeatures && (
                <div className="pt-6 space-y-2">
                  <p className="text-purple-400 dark:text-purple-500/60 text-xs">$ cat /etc/bgctub/features.conf</p>
                  <div className="pl-4 border-l-2 border-purple-300 dark:border-purple-500/20 space-y-3 pt-2">
                    {features.map((f, i) => (
              ...
              {showMission && (
                <div className="pt-6 space-y-4">
                  <p className="text-purple-400 dark:text-purple-500/60 text-xs">$ cat /etc/bgctub/purpose.md</p>
                  <div className="pl-4 border-l-2 border-purple-300 dark:border-purple-500/20 space-y-4 pt-2">
                    <div className="text-purple-500 dark:text-purple-400/70 text-xs">
                      Founded: December 19, 2023 | Members: 300+ | Status: Active
                    </div>
                    <div>
                      <p className="text-purple-600 dark:text-purple-400 font-bold mb-1">## Mission</p>
                    ...
                    <div>
                      <p className="text-purple-600 dark:text-purple-400 font-bold mb-1">## Vision</p>
              ...
              {showCta && (
                <div className="pt-8 space-y-4">
                  <p className="text-purple-400 dark:text-purple-500/60 text-xs">$ ./ready_to_join.sh</p>
                  <div className="p-4 rounded-lg border border-purple-200 dark:border-purple-500/20 bg-purple-100/50 dark:bg-purple-900/10 space-y-4">
                    <p className="text-purple-700 dark:text-purple-300 font-bold text-lg">Ready to Start Your Journey?</p>
                    <p className="text-purple-600/70 dark:text-purple-200/60 text-sm">Join BGCTUB IT Club today and become part of a thriving community of tech enthusiasts.</p>
                    <div className="flex flex-wrap gap-3">
                      ...
                      <Link href="/login">
                        <Button variant="outline" className="border-purple-300 dark:border-purple-500/30 text-purple-700 dark:text-purple-300 hover:bg-purple-100 dark:hover:bg-purple-500/10 d5-mono rounded-md">
          ...
          <div className="mt-8 grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              ...
              <div key={s.label} className="p-4 rounded-lg border border-purple-200 dark:border-purple-500/15 bg-purple-100/50 dark:bg-purple-900/10 text-center">
                <div className="text-2xl font-extrabold text-purple-600 dark:text-purple-400">{s.value}</div>
                <div className="text-xs text-purple-400 dark:text-purple-500/50 tracking-widest uppercase mt-1">{s.label}</div>
              </div>
            ))}
          </div>
        </main>
